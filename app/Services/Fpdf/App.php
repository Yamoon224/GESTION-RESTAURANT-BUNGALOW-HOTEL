...
class App extends Fpdf
{
    ...
    function receipt($header, $data, $others = NULL)
    {
        ...
        {
            $this->setX(2);
            $this->cell($width[0], 9, utf8_decode($row['product']['name']), '1', 0, 'C');
            $this->cell($width[1], 9, utf8_decode(moneyFormat($row['price'])), '1', 0, 'C');
            $this->cell($width[2], 9, utf8_decode($row['qty']), '1', 0, 'C');
            $this->cell($width[3], 9, utf8_decode(moneyFormat($row['amount'])), '1', 0, 'C');
            $this->Ln();
        }
        $this->Ln(2);
